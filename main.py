...
def create_ring_data(participants, data, current_index):
...
RING_PIPELINE_FOLDER: Path = Path(client.datasite_path) / "app_pipelines" / "ring"
RUNNING_FOLDER: Path = RING_PIPELINE_FOLDER / "running"
DONE_FOLDER: Path = RING_PIPELINE_FOLDER / "done"
SECRET_FILE: Path = RING_APP_PATH / "secret.json"
DATA_TEMPLATE_FILE: Path = RING_APP_PATH / "data.json"
...
participants, data, current_index = load_ring_data(file_path)
ring_length = len(participants)
data += my_secret
next_index = current_index + 1

if next_index < ring_length:
    next_person = participants[next_index]
    new_ring_data = create_ring_data(participants, data, next_index)
    ...
    final_ring_data = create_ring_data(participants, data, current_index)
